...
func openConn(t *testing.T, addr string, options ...goftp.DialOption) (*ftpMock, *goftp.ServerConn) {
	...
	c, err := goftp.Dial(mock.Addr(), options...)
	if err != nil {
		t.Fatal(err)
	}
...
func closeConn(t *testing.T, mock *ftpMock, c *goftp.ServerConn, commands []string) {
